        ...
        <section class="content">
            ...
            <div class="card">
                ...
                <div class="filter-container">
                    <label for="genero" class="d-inline-block ml-4 mr-2">Filtrar por Gênero:</label>
                    <select id="genero" class="form-control form-control-sm d-inline-block" style="width: auto;">
                        ...
                        <option value="baseado_em_fatos_reais">Baseado em Fatos Reais</option>
                    </select>
                </div>
                    ...
                    <table id="example1" class="table table-bordered table-striped" language="pt-br">
                        ...
                        @foreach ($livros as $v)
                            <tr>
                                ...
                                <td>{{ $v->status }}</td>
                                <td>
                                    <a href="{{ route('editar_livro', $v->id) }}">
                                ...
                                <td>
                                    <form action="{{ route('excluir_livro', $v->id) }}" method="POST" style="display: inline;">
                                        @csrf
                                        @method('DELETE')
                                        <button type="submit"
                                ...
                                <td>
                                    <a href="{{ route('editar_livro', $v->id) }}">
                                        <button type="submit" class="btn btn-primary container d-flex justify-content-center align-items-center" title="Editar um agendamento border-radius 20px">
...
<script>
    $(document).ready(function () {
     ...
     $(document).ready(function() {
        ...
        // Evento de mudança do filtro de gênero
        $('#genero').change(function() {
        var genero = $(this).val(); // Obtém o valor selecionado no dropdown

        // Recarrega a tabela com os dados filtrados
        if (genero) {
        table.column(3).search(genero).draw(); // Filtra pela coluna "Gênero" (coluna 2)
    } else {
        table.column(3).search('').draw(); // Se não houver filtro, mostra todos os livros
    }
    });
    });
</script>
